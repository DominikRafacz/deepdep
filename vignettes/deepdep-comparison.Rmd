---
title: "Comparison of deepdep to simillar packages"
...
There are a few already existing solutions to the problem of visualizing dependenices. In this last section, we will compare `deepdep` to those solutions.
...
* Returned object is kept in a quite popular and easilly modifiable format.
...
* Graph can get very messy easilly and there is no built-in options to prevent it.
...
### Advantages
* It has a fantastic function to automatically generating whole raport or vignette on one package.
...
### Disadvantages:
* No built-in package support for other plots than base.
...
### Advantages:
* You can pass URL of any repositories, on the contray to our function, which allows only CRAN, Bioconductor or locally installed packages, but as other reposetories are not as commonly used. 
...
* This package is pretty outdated. It was created when R was not so popular (and its repositories were quite small) and not modificated much since then.
